...
deltaT = 0.001
t = pl.arange(0, 10 + 0.5*(deltaT), deltaT)                 # form time vector
u = pl.sin(10*(t))                 # form input
sys = control.tf(num,den)               # form system
tout, yout, xout = ss.lsim(sys,u,t)             # linear simulation
# plot response
pl.figure()
pl.plot(tout,yout)
pl.xlabel('Time (sec)')
...
pl.show()
pl.figure()
ii = pl.arange(9001,10001, dtype=int)
pl.plot(tout[ii],yout[ii],tout[ii],u[ii])
pl.xlabel('Time (sec)')
